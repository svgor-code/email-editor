        ...
        <>
          ...
          <Box m={1} mt={2}>
            ...
            <TextField
              variant="outlined"
              value={
                props.props.linkTarget === '_self' ? 'Same Tab' : 'New Tab'
              }
              onChange={(e) => {
                e.persist();
                setProp(
...
export function BackgroundAccordion({
  props,
  setProp,
  isSelfBg,
  defaultImage,
  ...
  const [isBgImage, setIsBgImage] = React.useState(
    Boolean(
      isSelfBg ? props.style.backgroundImage : props.parentStyle.backgroundImage
    )
  );
    ...
    <CustomAccordion
      ...
      preview={
        isBgImage ? (
          ...
          <CustomColorButton title="" value={getBg(false)} />
        )
      }
      children={
        <>
